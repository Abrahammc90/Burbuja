...
def test_calculate_fine_cell_densities_neighboring():
    ...
    grid.boundaries = np.array([4.0, 4.0, 4.0])
    ...
    # For each coarse cell, print density stats
    for coarse_idx in range(xcells * ycells * zcells):
        densities = grid.fine_densities[coarse_idx, :]
        print(f"Coarse cell {coarse_idx}: fine cell densities stats -> min: {np.min(densities):.4f}, max: {np.max(densities):.4f}, mean: {np.mean(densities):.4f}")
        assert np.all(densities > 0), f"Coarse cell {coarse_idx}: some fine cell densities are not positive"
    print("calculate_fine_cell_densities_neighboring test passed!")
...
def test_fine_cell_masses_superblock():
    ...
    print(f"Total mass in coarse cells: {total_mass} (should be {np.sum(masses)})")
    assert np.isclose(total_mass, np.sum(masses)), f"Expected {np.sum(masses)}, got {total_mass}"
    print("Total atoms stored should be {natoms}:", np.sum(grid.atoms_stored_per_cell))
    assert np.sum(grid.atoms_stored_per_cell) == n_atoms, "Expected 10 atoms stored in coarse cells"
    assert np.sum(grid.atom_masses) == np.sum(masses), "Expected 10.0 masses stored in coarse cells, you got " + str(np.sum(grid.atom_masses))
    ...
    print(np.sum(grid.fine_mass_array[13]), np.sum(masses))
    exit()
    # Visualize fine mass distribution for first coarse cell’s superblock
    for i in range(grid.fine_xcells * grid.fine_ycells * grid.fine_zcells):
        x = i // (grid.fine_ycells * grid.fine_zcells) * grid.grid_fine_space_x
        y = (i % (grid.fine_ycells * grid.fine_zcells)) // grid.fine_zcells * grid.grid_fine_space_y
        z = i % grid.fine_zcells * grid.grid_fine_space_z
        ...
        print(f"Fine cell {i} atom coordinates:\n{grid.atom_coords[0][i]}")
...
def test_calculate_coarse_cell_masses():
    ...
    print("Test passed!")
test_fine_cell_masses_superblock()