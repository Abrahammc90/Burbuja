...
class Bubble():
    ...
    def find(self,
        ...
        # Free the big boolean mask now
        del bubble_mask

        # ---- setup ----
        total_cells = xcells * ycells * zcells
        # use narrower ints if safe (halves temp memory for index arrays)
        if total_cells < (1 << 31):
            bubble_indices = bubble_indices.astype(np.int32, copy=False)

        # bit-packed mask: 8 cells per byte
        num_bytes = (total_cells + 7) // 8
        bitpacked = np.zeros(num_bytes, dtype=np.uint8)

        def _set_bits(flat_idx_np: np.ndarray):
            # flat_idx_np must be int64 or int32 on CPU
            byte_idx = flat_idx_np >> 3           # i // 8
            bit_idx  = flat_idx_np & 7            # i % 8
            # vectorized in-place OR; accumulates when multiple indices hit same byte
            np.bitwise_or.at(bitpacked, byte_idx, (1 << bit_idx).astype(np.uint8, copy=False))

        yz = ycells * zcells
        float_dtype = np.float32 if use_float32 else np.float64
        gsx = float_dtype(grid_space_x); gsy = float_dtype(grid_space_y); gsz = float_dtype(grid_space_z)

        print(8)
        self.atoms = {}  # NOTE: this can still be big; keep only if you really need it

        chunk = 200000  # tune for your RAM (smaller = safer)
        atom_id = 1
        n = int(bubble_indices.size)

        print(6.6)
        for start in range(0, n, chunk):
            end = min(start + chunk, n)
            idx = bubble_indices[start:end]            # view into bubble_indices

            # mark bit-packed mask for this chunk
            _set_bits(idx.astype(np.int64, copy=False))

            # 1D -> 3D indices (chunked)
            iz = idx % zcells
            temp = idx // zcells
            iy = temp % ycells
            ix = temp // ycells
            ...
            # coords for this chunk (vectorized, chosen precision)
            x_chunk = ix.astype(float_dtype, copy=False) * gsx + (gsx * 0.5)
            y_chunk = iy.astype(float_dtype, copy=False) * gsy + (gsy * 0.5)
            z_chunk = iz.astype(float_dtype, copy=False) * gsz + (gsz * 0.5)

            # PDB lines (still RAM-heavy if many atoms; but temporaries stay small)
            residue_id = self.total_residues
            for k in range(x_chunk.shape[0]):
                self.atoms[atom_id] = (
                    f"ATOM {atom_id:>6d}  BUB BUB  {residue_id:>4d}    "
                    f"{x_chunk[k]:>8.3f}{y_chunk[k]:>8.3f}{z_chunk[k]:>8.3f}  1.00  0.00\n"
                )
                atom_id += 1

            # free per-chunk temps early
            del ix, iy, iz, x_chunk, y_chunk, z_chunk, idx

        # free the big index array
        del bubble_indices

        print(9); print(10); print(11)

        # expose compact mask (no dense 3D bool to avoid OOM)
        self.bubble_mask_bitpacked = bitpacked
        self.bubble_mask_shape = (xcells, ycells, zcells)
        self.bubble_data = None  # <-- intentionally not allocating the giant dense mask
